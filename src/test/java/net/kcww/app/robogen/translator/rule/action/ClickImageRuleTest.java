...
public class ClickImageRuleTest extends AbstractActionRuleTest {

    @InjectMocks
    private ClickImageRule clickImageRule;

    @ParameterizedTest
    @MethodSource("applicableTestCases")
    public void isApplicableTest(RelationModel relation) {
        runApplicableTest(relation, mock -> clickImageRule.isApplicable(mock));
    }

    @ParameterizedTest
    @MethodSource("inapplicableTestCases")
    public void isInapplicableTest(RelationModel relation) {
        runInapplicableActionTest(relation, mock -> clickImageRule.isApplicable(mock));
    }

    private static Set<Widget> widgets = Set.of(
            GwtWidgetEnum.IMAGE,
            VaadinWidgetEnum.IMAGE
    );
