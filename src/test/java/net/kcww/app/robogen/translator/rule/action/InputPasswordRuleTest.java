...
public class InputPasswordRuleTest extends AbstractActionRuleTest {

    @InjectMocks
    private InputPasswordRule inputPasswordRule;

    @ParameterizedTest
    @MethodSource("applicableTestCases")
    public void isApplicableTest(RelationModel relation) {
        runApplicableTest(relation, mock -> inputPasswordRule.isApplicable(mock));
    }

    @ParameterizedTest
    @MethodSource("inapplicableTestCases")
    public void isInapplicableTest(RelationModel relation) {
        runInapplicableActionTest(relation, mock -> inputPasswordRule.isApplicable(mock));
    }

    private static Set<Widget> widgets = Set.of(
            GwtWidgetEnum.PASSWORD_TEXTBOX,
            VaadinWidgetEnum.PASSWORD_FIELD
    );
